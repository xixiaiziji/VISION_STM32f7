...
void OV2640_I2C_Init(void)
{
    ...
    CAMERA_I2C_SDA_GPIO_CLK_ENABLE();

    /* 初始化GPIO */
    GPIO_InitStructure.Pin = CAMERA_I2C_SCL_PIN ;
    GPIO_InitStructure.Mode = GPIO_MODE_AF_OD;
    GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;
    GPIO_InitStructure.Pull = GPIO_NOPULL;
    GPIO_InitStructure.Alternate = CAMERA_I2C_SCL_AF;
    HAL_GPIO_Init(CAMERA_I2C_SCL_GPIO_PORT, &GPIO_InitStructure);

    GPIO_InitStructure.Pin = CAMERA_I2C_SDA_PIN ;
    GPIO_InitStructure.Alternate = CAMERA_I2C_SDA_AF;
    HAL_GPIO_Init(CAMERA_I2C_SDA_GPIO_PORT, &GPIO_InitStructure);

    /*初始化I2C模式 */
    I2C_Handle.Instance              = CAMERA_I2C;
    I2C_Handle.Init.Timing           = 0x90912732;
...
void OV2640_ReadID(OV2640_IDTypeDef *OV2640ID)
{
  ...
  {
    CAMERA_DEBUG("%x%x",OV2640ID->PIDH ,OV2640ID->PIDL);
  }
  else
  {
...
void OV2640_DCMI_Config(void) 
{
 ...
 // OV2640_DMA_Config(LCD_FRAME_BUFFER_LAYER0,lcd_width*lcd_height/2);



//  HAL_DCMI_ConfigCROP(&DCMI_Handle,0,0,800*2 - 1,480*2 - 1);
...
//  while (DMA_GetCmdStatus(DMA2_Stream1) != DISABLE){}	
  ...
  //DMA_Handle.Init.DMA_PeripheralBaseAddr = DCMI_DR_ADDRESS;	//DCMI数据寄存器地址
  //DMA_Handle.Init.DMA_Memory0BaseAddr = DMA_Memory0BaseAddr; //DMA传输的目的地址(传入的参数)	
  DMA_Handle_dcmi.Init.Direction = DMA_PERIPH_TO_MEMORY;
  //DMA_Handle.Init.DMA_BufferSize =DMA_BufferSize; 						//传输的数据大小(传入的参数)
...
void OV2640_DMA_M2M_Config(uint32_t v_Addr,uint32_t lcd_Addr,uint32_t DMA_BufferSize)
{
 /* 相关宏定义，使用存储器到存储器传输必须使用DMA2 */
  ...
  DMA_Handle.Instance = DMA2_Stream0;/* DMA数据流通道选择 */
  DMA_Handle.Init .Channel = DMA_CHANNEL_1;  
  
  DMA_Handle.Init.Direction = DMA_MEMORY_TO_MEMORY;   /* 存储器到存储器模式 */
  DMA_Handle.Init.PeriphInc = DMA_PINC_ENABLE;         /* 源地址自增 */
  DMA_Handle.Init.MemInc = DMA_MINC_ENABLE;            /* 目标地址自增 */
  DMA_Handle.Init.PeriphDataAlignment = DMA_MDATAALIGN_HALFWORD;/* 源数据是字大小(16位) */
  DMA_Handle.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;   /* 目标数据也是字大小(16位) */
  DMA_Handle.Init.Mode = DMA_NORMAL;                        /* 一次传输模式，存储器到存储器模式不能使用循环传输 */
  DMA_Handle.Init.Priority = DMA_PRIORITY_HIGH;             /* DMA数据流优先级为高 */
  DMA_Handle.Init.FIFOMode = DMA_FIFOMODE_DISABLE;          /* 禁用FIFO模式 */    
  DMA_Handle.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;
  DMA_Handle.Init.MemBurst = DMA_MBURST_SINGLE;             /* 单次模式 */
  DMA_Handle.Init.PeriphBurst = DMA_PBURST_SINGLE;          /* 单次模式 */
 
  HAL_DMA_Init(&DMA_Handle);                                 /* 完成DMA数据流参数配置 */
  DMA_status = HAL_DMA_Start(&DMA_Handle,(uint32_t)v_Addr,(uint32_t)lcd_Addr,DMA_BufferSize);
}
...
//    I2Cx_WriteMultiple(&I2C_Handle, OV2640_DEVICE_WRITE_ADDRESS, (uint16_t)Addr, I2C_MEMADD_SIZE_8BIT,(uint8_t*)&Data, 1);
  ...
  status = HAL_I2C_Mem_Write(&I2C_Handle, OV2640_DEVICE_WRITE_ADDRESS, (uint16_t)Addr, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&Data, 1, 1000);
  
  /* Check the communication status */
  if(status != HAL_OK)
  {
    /* Re-Initiaize the I2C Bus */
    I2Cx_Error_Handle(&I2C_Handle);
  }
  return status;
}
...
//    I2Cx_ReadMultiple(&I2C_Handle, OV2640_DEVICE_READ_ADDRESS, Addr, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&Data, 1);
  ...
  status = HAL_I2C_Mem_Read(&I2C_Handle, OV2640_DEVICE_READ_ADDRESS, (uint16_t)Addr, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&Data, 1, 1000);
...
{  
    ...
    DCMI->CR|=DCMI_CR_ENABLE;          //DCMI捕获使能
}
